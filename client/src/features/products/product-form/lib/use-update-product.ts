...
import { usePutProductsId } from "@/shared/api/generated/products/products";
import {
...
const useUpdateProduct = () => {
  ...
  const { mutateAsync } = usePutProductsId();

  const updateProduct = async (id: number, values: ProductFormValues) => {
    setIsPending(true);
    try {
      ...
      toast.success(t("form.updateSuccess", { name: product.name }));
